...
namespace ORB_SLAM2
{
	  // 默认初始化函数 单词表文件(txt/bin文件) 配置文件 传感器(单目、双目、深度) 是否可视化
	  ...
	  {
	      // 输出欢迎信息
	      cout << endl << "ORB-SLAM2 单目双目深度相机 SLAM" << endl <<
	      ...
	      {
			cerr << "打不开配置文件:  " << strSettingsFile << endl;
			exit(-1);
	      }
	      
		  // 配置文件中读取点云精度设置 for point cloud resolution
	  ...
	  // 单目相机跟踪 RGB图 时间戳 返回相机位姿==========================================================================
	  cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp)
	  {
	      if(mSensor!=MONOCULAR)
	      {
		  	cerr << "调用了单目跟踪 TrackMonocular 但是输入的传感器却不是单目 Monocular." << endl;
		  	exit(-1);
	      }
    	  ...
    	  // 1. 模式变换的检测  跟踪+定位 or 跟踪+定位+建图
	      {
		  	...
		  	if(mbActivateLocalizationMode)
		  	{
		      	mpLocalMapper->RequestStop();		// 停止建图线程
		      	// 等待建图线程停止
		      	while(!mpLocalMapper->isStopped())
		      	{
			  		usleep(1000);
	  ...
	  {
	      mpLocalMapper->RequestFinish();	// 完成 关闭建图线程
	      mpLoopCloser->RequestFinish();   	// 完成 关闭闭环检测线程
	      if(mpViewer)						// 完成 关闭可视化线程
	      {
